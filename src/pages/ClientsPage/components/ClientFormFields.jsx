		...
		{fields.map(({ name, icon, type, placeholder, required }) => (
			<div key={name} className="form-group flex items-center gap-4">
				{React.cloneElement(icon, {
					className: 'text-blue-900 text-3xl dark:text-blue-500',
				})}
				<input
					type={type}
					name={name}
					...
					className="flex-1 p-2 border rounded-md dark:bg-gray-800 dark:border-blue-700 dark:text-gray-200 focus:border-blue-700 focus:border-2 focus:outline-none active:border-blue-700"
					required={required}
				/>
			</div>
		))}
